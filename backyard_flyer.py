...
class BackyardFlyer(Drone):
    ...
    def state_callback(self):
        ...
        if not self.in_mission:
            return
        if (self.flight_state == States.MANUAL and self.guided==False):
            self.arming_transition()
        elif (self.flight_state == States.ARMING and self.guided==True):
            if self.armed:
                self.takeoff_transition()
        elif (self.flight_state == States.DISARMING and self.guided==True):
            if not self.armed:
                self.manual_transition()

    def calculate_box(self):

        self.all_waypoints = [[20.0,0.0,self.target_position[2]],
                              [20.0,20.0,self.target_position[2]],
                              [0.0,20.0,self.target_position[2]],
                              [0.0,0.0,self.target_position[2]]] 
